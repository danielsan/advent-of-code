...
async function main () {
  ...
  const handsAndBids = rawLines.map(line => {
    ...
    if (handMap.has('J') && handMap.size > 1) {
      const Jval = handMap.get('J')
      handMap.delete('J')
      const [[higherCard, higherVal]] = [...handMap.entries()].sort((a, b) => b[1] - a[1])
      handMap.set(higherCard, higherVal + Jval)
    }
